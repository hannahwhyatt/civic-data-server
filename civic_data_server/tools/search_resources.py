...
def register(mcp):
    ...
    async def search_resources(
        query: Annotated[
            str,
            Field(description="The query to search for resources. This must be a single keyword.")
        ]
        ) -> str:
        ...
        path = f"{ckan_api_base_url}/action/resource_search?query=name:{query}"
        ...
        try:
            ...
            if data.get("success") == True:
                return format_resource_search_response(data)
            else:
                return f"Error retrieving resource search results: {data.get('error')}"
        except requests.RequestException as e:
            return f"Error retrieving resource search results: {str(e)}"